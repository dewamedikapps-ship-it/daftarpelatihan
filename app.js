...
CSS += "\n.dm-kota-kelas{display:inline-block;margin-top:4px;font-size:11px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:var(--dm-blue-600,#183080);background:var(--dm-blue-100,#E7ECFA);border-radius:999px;padding:2px 9px;}\n.dm-pilih-kelas{max-width:340px;}\n.dm-pilih-kelas-on{border-color:var(--dm-blue-500,#2141A6);background:var(--dm-blue-050,#F2F5FD);font-weight:600;}\n";
CSS += "\n.dm-kota-peserta{display:flex;align-items:center;gap:4px;margin-top:2px;color:var(--dm-text,#102A33);font-weight:600;}\n";
/* Foto kegiatan di kartu kelas. */
CSS += "\n\n/* ---------- Foto kegiatan di kartu kelas ---------- */\n.dm-kelas::before{display:none;}\n.dm-kelas-foto{position:relative;aspect-ratio:16/9;overflow:hidden;background:#0F2237;}\n.dm-kelas-foto img{display:block;width:100%;height:100%;object-fit:cover;\n  transition:transform 400ms cubic-bezier(.22,.61,.36,1);}\n.dm-kelas:hover:not(.dm-kelas-off) .dm-kelas-foto img{transform:scale(1.04);}\n.dm-kelas-off .dm-kelas-foto img{filter:grayscale(.7);}\n.dm-terpilih{z-index:3;}\n@media (prefers-reduced-motion: reduce){ .dm-kelas-foto img{transition:none;} }\n";
/* ============================================================
...
function kotaKelas(k) {
  if (!k) return "";
  if (k.kota && String(k.kota).trim()) return String(k.kota).trim();
  for (const bagian of [k.judul, k.lokasi]) {
    for (const [nama, pola] of KOTA_DIKENAL) if (pola.test(String(bagian || ""))) return nama;
  }
  return "";
}

/* Foto kartu kelas — dipilih otomatis dari jenis pelatihan.
   Setiap kelas yang dibuka di back office langsung tampil dengan foto
   dokumentasi kegiatan DEWAMEDIK yang sesuai jenisnya, tanpa perlu
   diatur satu per satu. Kartu yang bersebelahan diberi foto berbeda.
   Menambah foto: taruh berkasnya di folder foto-kelas lalu tambahkan
   namanya ke daftar di bawah ini. */
const FOTO_KELAS = {
  BTCLS: ["foto-kelas/btcls-1.jpg", "foto-kelas/btcls-trauma.jpg", "foto-kelas/rjp-1.jpg", "foto-kelas/btcls-cedera.jpg"],
  ACLS: ["foto-kelas/acls-1.jpg", "foto-kelas/acls-aed.jpg", "foto-kelas/rjp-1.jpg"],
  EKG: ["foto-kelas/acls-aed.jpg", "foto-kelas/acls-1.jpg", "foto-kelas/umum-ujian.jpg"],
  PKID: ["foto-kelas/darurat-1.jpg", "foto-kelas/darurat-2.jpg", "foto-kelas/btcls-trauma.jpg"],
...
function kelompokFoto(t) {
  const jenis = String(t && t.jenis || "").toUpperCase().trim();
  if (FOTO_KELAS[jenis]) return FOTO_KELAS[jenis];
  const teks = (jenis + " " + String(t && t.judul || "")).toUpperCase();
  if (/BTCLS|BHD|BLS|TRAUMA|RJP|CPR/.test(teks)) return FOTO_KELAS.BTCLS;
  if (/ACLS|CARDIO|JANTUNG|RESUSITASI/.test(teks)) return FOTO_KELAS.ACLS;
  if (/EKG|ECG|ELEKTROKARDIO|ARITMIA/.test(teks)) return FOTO_KELAS.EKG;
  if (/GAWAT|DARURAT|EMERGENC|PKID|KKMN|BONELS/.test(teks)) return FOTO_KELAS.PKID;
  if (/PPI|INFEKSI/.test(teks)) return FOTO_KELAS.PPIK;
  return FOTO_KELAS.UMUM;
}
function fotoUntukDaftar(daftar) {
  const hasil = [];
  const hitung = new Map();
  (daftar || []).forEach((t, i) => {
    const sendiri = t && (t.foto || t.gambar || t.image_url);
    if (sendiri) { hasil.push(sendiri); return; }
    const kelompok = kelompokFoto(t);
    let n = hitung.get(kelompok) || 0;
    let pilih = kelompok[n % kelompok.length];
    for (let k = 1; k < kelompok.length && pilih === hasil[i - 1]; k++) {
      n++;
      pilih = kelompok[n % kelompok.length];
    }
    hitung.set(kelompok, n + 1);
    hasil.push(pilih);
  });
  return hasil;
}
...
function pesanTerverifikasi(r, setelan) {
  ...
  if (Number(r.diskon) > 0) {
    b.push(`Member Benefit   : ${rp(r.diskon)} (${r.persen}%${r.voucher_kode ? " · " + r.voucher_kode : ""})`);
  }
  if (r.minta_invoice) {
    b.push("");
...
function Pendaftaran({
  ...
  const formLengkap = !!training && emailValid(data.email) && /^[0-9]{16}$/.test(data.nik) && !!data.nama.trim() && /^[0-9+\-\s]{8,}$/.test(data.hp) && !!data.instansi.trim() && !!data.kota.trim() && !!data.plataran && !!berkas.foto && !!berkas.transfer;
  ...
  if (!data.kota.trim()) kurang.push("Kota/kabupaten asal");
  ...
  async function pakaiKode() {
    ...
    if (h && h.ok) {
      setKodeManual(h);
      setPesanKode(`Member Benefit ${h.persen}% terpasang.`);
    } else {
      setKodeManual(null);
  ...
  if (hasil) {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "dm-mono"
    }, rp(hasil.harga_awal))), Number(hasil.diskon) > 0 ? /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", null, "Member Benefit ", hasil.persen, "%"), /*#__PURE__*/React.createElement("b", {
  ...
  }, /*#__PURE__*/React.createElement("span", null, "Member Benefit", dipakai ? ` · ${dipakai.persen}%` : ""), /*#__PURE__*/React.createElement("b", null, potongan > 0 ? "− " + rp(potongan) : rp(0))), /*#__PURE__*/React.createElement("div", {
    className: "dm-baris"
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("article", {
      ...
      className: "dm-kelas-foto",
      "aria-hidden": "true"
    }, /*#__PURE__*/React.createElement("img", {
      src: fotoUntukDaftar(kelasTampil)[iKelas],
      alt: "",
      loading: "lazy",
      decoding: "async"
    ...
    className: "dm-hint"
  }, "Biaya di atas belum memperhitungkan Member Benefit. Member Benefit terpasang otomatis pada langkah pembayaran bila Anda memenuhi syaratnya."), /*#__PURE__*/React.createElement("div", {
    className: "dm-row dm-row-akhir"
  }, /*#__PURE__*/React.createElement("button", {
    className: "dm-btn",
    ...
    className: "dm-baris"
  }, /*#__PURE__*/React.createElement("span", null, "Member Benefit ", dipakai ? dipakai.persen + "%" : ""), /*#__PURE__*/React.createElement("b", {
  ...
  }, "Member Benefit ", dipakai.persen, "% terpasang") : /*#__PURE__*/React.createElement("p", {
  ...
  }, bukaRingkas ? "Tutup rincian" : "Rincian") : null), training && sisaBayar > 0 ? /*#__PURE__*/React.createElement("p", {
...
function TabPendaftar({
  ...
  const [cari, setCari] = useState("");
  /* Pilihan kelas: "" = semua, "kota:<nama>" = semua kelas di kota itu,
     "id:<uuid>" = satu kelas. Tabel, angka ringkasan, dan unduhan Excel
     ikut pilihan ini. */
  ...
  async function muat() {
    const {
      data,
      error
    } = await SB.from("pendaftaran").select("*, pelatihan(id, judul, jenis, tanggal_mulai, tanggal_selesai, lokasi, kota, wa_grup)").order("dibuat", {
      ascending: false
    });
  ...
  /* Kelompokkan kelas per kota pelaksanaan untuk pilihan di atas tabel. */
  const petaKelas = new Map();
  baris.forEach(r => {
    const k = r.pelatihan;
    if (!k) return;
    const kota = kotaKelas(k) || "Lainnya";
    if (!petaKelas.has(kota)) petaKelas.set(kota, new Map());
    const m = petaKelas.get(kota);
    if (!m.has(k.id)) m.set(k.id, { k, n: 0 });
    if (!mati(r)) m.get(k.id).n++;
  });
  const grupKota = [...petaKelas.entries()].sort((a, b) => a[0] === "Lainnya" ? 1 : b[0] === "Lainnya" ? -1 : a[0].localeCompare(b[0], "id"))
    .map(([kota, m]) => [kota, [...m.values()].sort((a, b) => String(b.k.tanggal_mulai || "").localeCompare(String(a.k.tanggal_mulai || "")))]);
  const cocokKelas = r => {
    if (!fKelas) return true;
    if (fKelas.startsWith("id:")) return !!r.pelatihan && r.pelatihan.id === fKelas.slice(3);
    if (fKelas.startsWith("kota:")) return (kotaKelas(r.pelatihan) || "Lainnya") === fKelas.slice(5);
    return true;
  };
  let labelPilihan = "";
  if (fKelas.startsWith("kota:")) labelPilihan = fKelas.slice(5);
  else if (fKelas.startsWith("id:")) {
    const k = (baris.find(r => r.pelatihan && r.pelatihan.id === fKelas.slice(3)) || {}).pelatihan;
    labelPilihan = k ? k.judul : "";
  }
  const dasar = baris.filter(cocokKelas);
  const aktif = dasar.filter(r => !mati(r));
  const tampil = dasar
    .filter(r => filter === "aktif" ? !mati(r) : filter === "semua" ? true : r.status === filter)
    .filter(r => cari ? (r.nama + r.email + r.nomor + r.nik + (r.kota || "")).toLowerCase().includes(cari.toLowerCase()) : true);
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    className: "dm-mono"
  }, aktif.length), dasar.length - aktif.length > 0 ? /*#__PURE__*/React.createElement("p", {
    className: "dm-hint"
  }, dasar.length - aktif.length, " batal/refund") : null), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", null, "Perlu diperiksa"), /*#__PURE__*/React.createElement("b", {
    className: "dm-mono"
    ...
    className: "dm-input",
    placeholder: "Cari nama, email, NIK, kota, nomor registrasi",
    value: cari,
    onChange: e => setCari(e.target.value)
  }), /*#__PURE__*/React.createElement("select", {
    className: "dm-input dm-pilih-kelas" + (fKelas ? " dm-pilih-kelas-on" : ""),
    value: fKelas,
    "aria-label": "Pilih kelas",
    onChange: e => setFKelas(e.target.value)
  }, /*#__PURE__*/React.createElement("option", {
    value: ""
  }, "Semua kelas"), grupKota.map(([kota, daftar]) => /*#__PURE__*/React.createElement("optgroup", {
    key: kota,
    label: kota
  }, /*#__PURE__*/React.createElement("option", {
    value: "kota:" + kota
  }, "Semua kelas di " + kota + " (" + daftar.reduce((a, b) => a + b.n, 0) + ")"), daftar.map(({ k, n }) => /*#__PURE__*/React.createElement("option", {
    key: k.id,
    value: "id:" + k.id
  }, k.judul + (k.tanggal_mulai ? " \u00b7 " + rentang(k.tanggal_mulai, k.tanggal_selesai) : "") + " (" + n + ")"))))), /*#__PURE__*/React.createElement("select", {
    className: "dm-input",
    value: filter,
    ...
    className: "dm-hint"
  }, r.email), r.kota ? /*#__PURE__*/React.createElement("p", {
    className: "dm-hint dm-kota-peserta"
  }, /*#__PURE__*/React.createElement(Ikon, {
    nama: "lokasi",
    ukuran: 13,
    kelas: "dm-svg-samar"
  }), "Asal: " + r.kota) : null), /*#__PURE__*/React.createElement("td", null, r.pelatihan ? r.pelatihan.judul : "—", r.pelatihan && r.pelatihan.tanggal_mulai ? /*#__PURE__*/React.createElement("p", {
    className: "dm-hint"
    ...
    className: "dm-hint"
  }, "Instansi"), /*#__PURE__*/React.createElement("p", null, detail.instansi), /*#__PURE__*/React.createElement("p", {
    className: "dm-hint"
...
function TabJadwal({
  ...
  return /*#__PURE__*/React.createElement("section", {
    ...
    hint: "Dipakai untuk memilah data pendaftar per kota, mis. Cirebon, Semarang, Tangerang."
  }, /*#__PURE__*/React.createElement("input", {
    className: "dm-input",
    value: edit.kota || "",
    maxLength: 60,
...
function TabVoucher({
  ...
  }, "Kode member bersyarat berkas terpasang otomatis begitu peserta mengunggah berkasnya. Bila ada dua kode yang cocok, peserta mendapat Member Benefit yang paling besar."), /*#__PURE__*/React.createElement("div", {
    className: "dm-tabel-bungkus"
  }, /*#__PURE__*/React.createElement("table", {
    className: "dm-tabel"
  }, /*#__PURE__*/React.createElement("thead", null, /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("th", null, "Kode"), /*#__PURE__*/React.createElement("th", null, "Member Benefit"), /*#__PURE__*/React.createElement("th", null, "Untuk"), /*#__PURE__*/React.createElement("th", null, "Syarat"), /*#__PURE__*/React.createElement("th", null, "Kuota"), /*#__PURE__*/React.createElement("th", null, "Status"), /*#__PURE__*/React.createElement("th", null))), /*#__PURE__*/React.createElement("tbody", null, baris.map(v => /*#__PURE__*/React.createElement("tr", {
    key: v.id
